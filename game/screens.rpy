...
screen nav_img_btn(img, act):
    imagebutton:
        idle At(img, nav_btn_idle)
        hover At(img, nav_btn_hover)
        action act
        focus_mask True


transform gallery_btn_idle:
    xzoom 0.5
    yzoom 0.5
    matrixcolor BrightnessMatrix(0.0)
...
screen gallery_img_btn(img, act):
    imagebutton:
        idle At(img, gallery_btn_idle)
        hover At(img, gallery_btn_hover)
        action act
        focus_mask True

transform gallery_box:
    xzoom 0.2
    yzoom 0.2
...
init python:
    #영구저장
    if not hasattr (persistent, "gallery_unlocks"):
        persistent.gallery_unlocks = set()

    #최신해금이 앞으로 오도록
    if not hasattr (persistent, "gallery_unlock_order"):
        persistent.gallery_unlock_order = []

    # unlocks: set이어야 함
    if getattr(persistent, "gallery_unlocks", None) is None:
        persistent.gallery_unlocks = set()

    # order: list여야 함
    if getattr(persistent, "gallery_unlock_order", None) is None:
        persistent.gallery_unlock_order = []

    def unlock_gallery(illust_id):
        """
        스토리 진행 중 호출해서 일러스트 해금.
        예: $ unlock_gallery("jin_01")
        """
        if illust_id not in persistent.gallery_unlocks:
            persistent.gallery_unlocks.add(illust_id)

        if illust_id in persistent.gallery_unlock_order:
            persistent.gallery_unlock_order.remove(illust_id)

        persistent.gallery_unlock_order.insert(0, illust_id)
        renpy.save_persistent()

## 갤러리 데이터 = 탭이미지 + 일러스트목록 정의
init python:
    GALLERY_TABS = {
        "백담우": "images/gallery/back-part.webp",
        "진효":   "images/gallery/jin-part.webp",
        "화야진": "images/gallery/hwa-part.webp",
        "종합":   "images/gallery/all-part.webp",
    }

    UNKNOWN_ILLUST = "images/gallery/illustration/unknown.webp"

    # 여기에 실제 파일 경로를 채우세요.
    # id는 "해금 키", char는 분류 탭, file은 실제 이미지 파일
    GALLERY_ITEMS = [
        {"id":"back_01", "char":"백담우", "file":"images/gallery/illustration/back_01.webp"},
        {"id":"jin_01",  "char":"진효",   "file":"images/gallery/illustration/jin_01.webp"},
        {"id":"hwa_01",  "char":"화야진", "file":"images/gallery/illustration/hwa_01.webp"},
        # ...
    ]
...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            use nav_img_btn("images/buttons/불러오기.webp", ShowMenu("load"))
            use nav_img_btn("images/buttons/갤러리.webp", ShowMenu("gallery_menu"))
            use nav_img_btn("images/buttons/설정.webp", ShowMenu("preferences"))
            use nav_img_btn("images/buttons/연화록사.webp", ShowMenu("world_history"))

            if renpy.variant("pc"):
                use nav_img_btn("images/buttons/종료하기.webp", Quit(confirm=True))

        else:

            use nav_img_btn("images/buttons/저장하기.webp", ShowMenu("save"))
            use nav_img_btn("images/buttons/불러오기.webp", ShowMenu("load"))
            use nav_img_btn("images/buttons/환경설정.webp", ShowMenu("preferences"))
            use nav_img_btn("images/buttons/메인메뉴.webp", MainMenu())

            if renpy.variant("pc"):
                use nav_img_btn("images/buttons/게임종료.webp", Quit(confirm=True))
...
screen world_history() :
    tag menu
    add "images/background/chapter_background.webp"

    default wh_show_map = False
    ...
    frame:
        ...
        vbox:
            ...
            text "연화록사" size 46 color "#000000" xalign 0.5
            null height 30
            ...
            viewport:
                ...
                vbox:
                    ...
                    text "대륙의 구조 (청월국 멸망 전 시점)" size 28 color "#000000"
                    text "대륙은 수많은 국가들로 이루어져 있다." size 24 color "#000000"

                    null height 20

                    text "대륙 중앙 — 청월국 : 고산지대와 청명한 물길로 이루어진 오래된 왕조, 문화적·영적 권위를 지닌 국가였으나 전쟁에 취약했다." size 24 color "#000000"
                    null height 10
                    text "남동부의 길게 뻗은 반도 — 운상국: 바다를 지배하는 해상대국, 대륙 대부분의 무역길을 사실상 독점하는 상단국가." size 24 color "#000000"
                    null height 10
                    text "중앙 대륙 대부분을 장악한 신흥 강국 — 태휘국 : 군사력 중심의 신흥 제국. 청월국과 운상국 사이에 위치, 수십 개의 소국을 흡수하며 영토를 빠르게 넓혀갔다." size 24 color "#000000"

                    null height 20

                    text "그리고 이 세 나라 사이에는 여러 소국과 부족국가들이 완충지대를 이루며 존재했다. 하지만 태휘국이 이들을 차례로 복속시키면서 대륙의 균형은 완전히 무너졌다." size 24 color "#000000"
            ...
            hbox: # 버튼
                ...
                use nav_img_btn ("images/buttons/map_button.webp", SetScreenVariable("wh_show_map", True))
                use nav_img_btn ("images/buttons/back_button.webp", Return())
    ...
    if wh_show_map:
        button:
            ...
            action SetScreenVariable("wh_show_map", False)

        add "images/history/map.webp" xalign 0.5 yalign 0.5
...
screen gallery_menu():
    ...
    #상태
    default selected_character = "백담우"
    default  page = 0
    default  zoom_file = None

    # 설정
    $ items_per_page = 6

    # 해금된 id들을 "최신 해금 순"으로 정렬
    $ unlocked_ids = persistent.gallery_unlocks
    $ ordered_ids = [i for i in persistent.gallery_unlock_order if i in unlocked_ids]

    # 탭별 보여줄 id 목록 만들기
    $ tab_ids = []
    if selected_character == "종합":
        $ tab_ids = ordered_ids[:]  # 전부(해금된 것만)
    else:
        # 해당 캐릭터에 속하면서 해금된 것만
        $ tab_ids = [i for i in ordered_ids if any(x["id"] == i and x["char"] == selected_character for x in GALLERY_ITEMS)]

    # 페이지 슬라이싱
    $ total_pages = max(1, (len(tab_ids) + items_per_page - 1) // items_per_page)
    $ page = max(0, min(page, total_pages - 1))
    $ page_ids = tab_ids[page * items_per_page : (page + 1) * items_per_page]

    # id -> 파일 경로
    $ id_to_file = {x["id"]: x["file"] for x in GALLERY_ITEMS}
    ...
    fixed:
        ...
        add "images/UI/blank-brown-paper2.webp" xpos 0.1 ypos 0.15 xzoom 0.8 yzoom 0.8
        ...
        grid 3 2:
            ...
            # 실제 슬롯 6개를 항상 채움(빈칸은 unknown)
            for slot in range(items_per_page):
                $ is_locked = True
                $ f = UNKNOWN_ILLUST

                if slot < len(page_ids):
                    $ _id = page_ids[slot]
                    $ f = id_to_file.get(_id, UNKNOWN_ILLUST)
                    $ is_locked = False

                if is_locked:
                    add At(f, gallery_box)
                else:
                    imagebutton:
                        idle At(f, gallery_box)
                        hover At(f, gallery_box)
                        focus_mask True
                        action SetScreenVariable("zoom_file", f)
        ...
        # 뒤로가기(리액트의 back_button) :contentReference[oaicite:5]{index=5}
        hbox: # 버튼
                xalign 0.9
                yalign 0.92
                spacing 5
    ...
    # 확대 모달(리액트의 zoom-modal/zoom-image와 동일 역할) :contentReference[oaicite:6]{index=6}
    if zoom_file:
        button:
            background Solid("#ffffff88")  
            action SetScreenVariable("zoom_file", None)
            xfill True
            yfill True
